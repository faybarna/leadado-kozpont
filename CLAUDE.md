...
- **Végpontok:** `/subscribe`, `/unsubscribe`, `/test`, `/test-weekly`, `/send` (célzott token VAGY `"all"` broadcast), `/partners` (admin-gated partner-lista), `/changes` (admin nélkül — a "mit változott" jelzéshez, saját `seen:{token}` KV-alap, független a push `state:{token}`-től). **Cron:** `*/20 * * * *` pipeline-diff push; `0 6 * * 1` heti digest (csak ha `WEEKLY_ENABLED="true"`).
